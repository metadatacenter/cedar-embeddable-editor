...
const STANDALONE_COPY = fileURLToPath(new URL('./public/cedar-embeddable-editor.js', import.meta.url));
const STANDALONE_MANIFEST = fileURLToPath(new URL('./public/bundle-manifest.json', import.meta.url));
...
for (const hostFonts of [false, true]) {
  const COPY = hostFonts ? STANDALONE_COPY.replace('.js', '.host-fonts.js') : STANDALONE_COPY;
  const MANIFEST = hostFonts ? STANDALONE_MANIFEST.replace('.json', '.host-fonts.json') : STANDALONE_MANIFEST;
  if (!existsSync(COPY)) {
  ...
  try {
    output = resolveBuildOutput(
      hostFonts ? resolve(DEFAULT_DIST, '../cedar-embeddable-editor-host-fonts') : DEFAULT_DIST,
    );
  } catch (error) {
    ...
    if (hostFonts) die('host-font build output is missing. Run: npm run build:production');
    continue;
  }
